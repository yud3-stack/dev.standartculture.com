...
export function ProjectDetail({ locale, slug }: { locale: Locale; slug: string }) {
  ...
  return <><Navbar locale={locale} /><main className="mx-auto max-w-7xl px-6 pb-24 pt-16 md:px-10 md:pt-24"><p className="text-[11px] font-normal tracking-[0.2em] text-[#E73C29]">{text.project} {project.id}</p><h1 className="mt-6 max-w-4xl text-5xl font-semibold leading-[.95] tracking-[-.07em] md:text-8xl">{project.title}</h1><p className="mt-7 max-w-2xl text-lg leading-relaxed text-neutral-600">{project.description}</p>
    <div className="mt-16 flex aspect-[16/8] items-center justify-center rounded-2xl" style={{ backgroundColor: project.color }}></div>
    <section className="mt-24 grid gap-10 md:grid-cols-[1fr_2fr]"><p className="text-[13px] font-normal tracking-[.2em] text-[#E73C29]">{text.about}</p><div><h2 className="text-4xl font-semibold leading-[1] tracking-[-.06em] md:text-6xl">{text.heading}</h2><p className="mt-7 max-w-2xl leading-relaxed text-neutral-600">{project.description} {text.body}</p></div></section>
    ...
    <dl className="mt-20 grid gap-8 border-y border-neutral-200 py-8 sm:grid-cols-3">{[[text.year, project.year], [text.category, project.category], [text.services, project.services.join(", ")]].map(([term, value]) => <div key={term}><dt className="text-[10px] font-semibold tracking-[.18em] text-[#e63946]">{term}</dt><dd className="mt-3 font-bold">{value}</dd></div>)}</dl>
